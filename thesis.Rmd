...
for exploratory time series analysis & 80\%. Concept and developing software and writing first draft & (1) Rob J Hyndman, Concept and input into manuscript 10\% (2) Dianne Cook, Concept and input into manuscript 10\%  & \multicolumn{1}{c|}{N} \\
\hline
...
The Smart Grid, Smart City (SGSC) project (2010–2014) available through [Department of the Environment and Energy](https://data.gov.au/data/organization/doee) provides half-hourly data of over 13,000 household electricity smart meters distributed unevenly from October 2011 to March 2014 for some parts in Australia. Because of the increasing variety of customers, larger data sets contain more uncertainty about customer behavior. Household behavioral patterns vary significantly due to differences in size, location, and amenities such as solar panels, central heating, and air conditioning. For example, some families use a dryer, while others hang their clothes to dry. This could be reflected in their weekly profile. They may vary on a monthly basis, with some customers using more air conditioners or heaters than others despite having comparable electrical equipment and weather conditions. Some customers are night owls, while others are morning larks, which may show up in their daily profile. Customers' day-off energy consumption varies depending on whether they stay at home or go outside. The motivation for this work comes from the desire to provide methods to better understand heterogeneity in repetitive behavior that can arise across temporal dependencies.
...
Slicing and dicing data in all possible temporal scales, as suggested by EDA, can be a daunting task because it opens up a plethora of possibilities. This motivates the research presented in this thesis, which aims to provide a platform for systematically exploring periodicities in temporal data and supporting the discovery of regular patterns or anomalies, as well as the exploration of clusters of behaviors or the summarization of the behavior. The transition from linear to cyclic granularities results in restructured data, with each level of the time deconstructions corresponding to multiple values of the observed variable. For example, it is common to see aggregated usage across households, such as total kWh used each half-hour by state, because energy companies need to know the maximum loads they must plan for. However, studying overall energy consumption obscures usage distributions at finer scales, making it more difficult to find solutions to improve energy efficiency. Summarizing the probability distribution of these multiple observations to capture both shape and uncertainty could be a potential approach to understanding the underlying distribution of these observations. The research objectives presented in each chapter aid in the achievement of this goal.
...
With multiple time series available for each observation, the complexity of efficient exploration and visualization grows exponentially. In this case, conditional distributions include not only temporal dependency but also variables and their dependencies. This adds to the already high-dimensional data structures that result from studying distributions. Current research on visualizing multivariate data must account for time's inherent characteristics in order to efficiently explore distributions of multivariate time series data.
...
With the volume of data expected to increase in the future, potentially leading to increased variability in patterns, more experiments are needed to understand what sort of problems may arise with our current methodology and how to improve our current algorithm to address those issues. All of this must be done while keeping computational efficiency in mind, as this is critical when scaling up for analysis of large data sets.


### Develop inferential methods for the clusters

It is crucial to determine whether or not any detected clusters or patterns are statistically significant. In Chapter \@ref(ch:gracsr), we have used the permutation approach to detect if patterns are authentic or not across one or a pair of granularities. A similar method for finding significance across clusters is not appropriate if the observations are assumed to be independent. Most large temporal data sets gathered at fine temporal scales usually include several repetitive patterns. Therefore, the permutations used for comparisons must account that different temporal granularities have distinct conditional distributions.

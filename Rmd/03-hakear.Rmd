...
Let $v_{i, t}$ denote the electricity demand for the $i^{th}$ household in time period $t$. The series $\{v_{i, 1},\dots,v_{i,T}\}$ is the linear granularity corresponding to half-hour since the interval of the `tsibble` is 30 minutes. We consider coarser linear granularities like hour, day, week and month from the commonly used Gregorian calendar. From the four linear granularities of hour, day, week, and month, we obtain $N_C = 4\times 3/2 = 6$ cyclic granularities:  "hour_day", "hour_week", "hour_month", "day_ week", "day_month" and "week_month" abbreviated as *hod*, *how*, *hom*, *dow*, *dom* and *wom* respectively. Further, we add cyclic granularity day-type ("wknd wday") to capture weekend and weekday behavior (abbreviated as *wnwd*). Thus, seven cyclic granularities are considered to be of interest. The pairs of cyclic granularities ($C_{N_C}$) will have $7\times 6=42$ elements. The set of possible harmonies $H_{N_C}$ from $C_{N_C}$ are chosen by removing clashes using procedures described in @Gupta2021-gravitas. Table \ref{tab:tab-rank-8} shows $14$ harmony pairs that belong to $H_{N_C}$.

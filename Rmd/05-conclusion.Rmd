...
This chapter summarizes the thesis content, covering original contributions, software development, and future ideas.
...
Chapter \@ref(ch:gravitas)


This thesis presents a framework for visualizing and analyzing distributions of large temporal data by leveraging time characteristics. The building blocks of this framework are presented in each chapter. Exploratory time series analysis involves many iterations of finding and summarizing temporal dependencies. It is typical to divide time into years, months, weeks, days and so on to carry out inferences both across finer and coarser scales. The formalization of these temporal deconstructions (granularities) in literature are limited to linear time granularities like hours, days, weeks, and months which respect the linear progression of time and are non-repeating. Cyclic granularities which are repeating in nature are useful for finding patterns in temporal data. They can be circular, quasi-circular, or aperiodic. Examples of circular granularities are hour of the day and day of the week; an example of a quasi-circular granularity is day of the month; examples of aperiodic granularities are public holidays and school holidays. Time deconstructions can also be based on time hierarchy. Thus, single-order-up granularities like second of minute or multiple-order-up granularities like second of hour can be conceived. The definitions and rules defined in the literature for linear granularities are inadequate for describing different categories of cyclic granularities. Chapter \@ref(ch:gravitas) provides a formal characterization of cyclic granularities and provides tools to classify and compute possible cyclic granularities from an ordered (usually temporal) index. It also facilitates manipulation of single- and multiple-order-up time granularities through cyclic calendar algebra.

Visualizing probability distributions conditional on one or more cyclic granularities is an indispensable tool for exploration. However, firstly, there could be too many cyclic granularities to look at manually for comprehensive exploration and secondly, not all pairs of granularities can be effectively explored together. This chapter helps to list down the possible cyclic granularities that can be explored given a considered hierarchy and provides a recommendation whether a pair of granularities can be meaningfully plotted or analyzed together based on the synergy and levels of the cyclic granularities.

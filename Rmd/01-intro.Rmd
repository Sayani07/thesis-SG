...
The thesis is structured as follows. Chapter \@ref(ch-gravitas) provides details of the cyclic granularities, different classes, and computation, and also its usage in exploratory time series analysis through applications. This is implemented in the R package `gravitas` (GRAnularity VIsualization for Time-series AnalySis). Chapter \@ref(ch-hakear) provides guidance on how to choose significant cyclic granularities, which are likely to have interesting patterns across its categories. This is available as the R package `hakear` (HArmonies KEeper And Rater). Chapter \@ref(ch-gracsr) provides methods to explore heterogeneity in repetitive behavior for multiple time series over multiple cyclic granularities. This is in the developing R package `gracsr` (Granularity CluStering in R). Chapter \@ref(ch-conclusion) summarizes the software tools developed for the work, and discusses future plans. Table \ref{tab:contribution-outline} details the publications, including my and my fellow co-authors contributions.

\begin{table}
\centering\footnotesize\tabcolsep=0.12cm
\caption{Table outlining the main chapters' co-authorship arrangements.}
\label{tab:contribution-outline}

...
Done.
Added "for a continuous univariate dependent variable" in the last paragraph of Introduction of Chapter 3.
...
Added text in the caption of Figure 3.3(a) and "Notation" sub-section to make it clear.
...
Rephrased the simulation design and results corresponding to $m=2$. Similar design and results for $m=1$, although important, are not included in the main chapter but in the supplements. Simulation results involving different underlying distributions are also presented in the supplementary paper.

> Q20. In Figure 3.5 the axis tick labels should be smaller. The blue and orange marks are hard to see. Maybe
show fewer panels?

Changed. Axis tick labels made smaller and space between facets minimised to allow for more space.
...
Changed. Axis tick labels made smaller and the rug plots are removed to make the display less busy.
...
Rephrased the text in the  Data pre-processing section to make it easier to understand.  

Figure 4.1 labels changed to make it consistent with the text.
...
No change. I had mentioned that $D$ stands for Jensen-Shannon distances.
...
The table format and caption changed to address this comment.
...
Clarification: Various range of values of $R$ and $T$ are considered as stated in Table 4.1.
...
Changed. "we have assumed the time series to be stationary" has been changed to "the conditional distributions are assumed to remain constant for the observation period" in the Discussion section.
...
A similar question has been answered for the next reviewer.
...
Added under Thesis structure in Introduction.
...
- Introduction (last but one paragraph) to incorporate comments on existing distance measures and their limitations

- "Distance between distributions" section to mention why we chose we chose Jensen-Shannon distances and some details about other distance measures.
...
Caption changed to illustrate the plot better. Added box borders in (a) such that (a) and (b) display the household ids in the same order as indicated by the same color of the line plot in (b) and box border in (a).
...
- To reflect various energy behavior, data for eight homes for Section 3.4 is acquired from friends and colleagues with different professions, lifestyles, and household sizes. This level of data is private, and we thank our friends and colleagues for agreeing to have their data used for this analysis. This is added in the first paragraph of Application.

- A paragraph is added to the Discussion section to incorporate comments on the computational time.
...
The Discussion section is split into (1) conclusions, and (2) limitations and future research (including a discussion on computational complexity) to address this comment.
